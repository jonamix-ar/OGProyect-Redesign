    ...
    <td class="first" rowspan="3">
        <div>
            <a href="javascript:void(0);" class="tooltip js_hideTipOnMobile" style="display: block;"
                onclick="cancelbuilding({{ $building_id }}, {{ $build_current }}, '{{ $build_cancel }}'}); return false;"
                title="{{ $build_cancel }}">
            ...
            <a href="javascript:void(0);" class="tooltip js_hideTipOnMobile abortNow"
                onclick="cancelbuilding({{ $building_id }}, {{ $build_current }}, '{{ $build_cancel }}'); return false;"
                title="{{ $build_cancel }}">
                <img src="{{ $img_path . 'layout/pixel.gif' }}" height="15"
                    width="15">
            </a>
        </div>
    </td>
    <td class="desc ausbau">{{ $ov_improve_to }}
        <span class="level">{{ $ov_level }} {{ $level }}</span>
    </td>
</tr>
<tr class="data">
    <td class="desc">{{ $ov_duration }}:</td>
</tr>
<tr class="data">
    <td class="desc timer">
        <span id="buildingCountdown">{{ $time_end_format }}</span>
    </td>
</tr>
<tr class="data">
    <td colspan="2">
...
<script>
    ...
    var restTimebuilding = {{ $build_time }} - Math.floor((Date.now() + window.timeDiff + window
            .timeZoneDiffSeconds *
            1000) /
        1000);
